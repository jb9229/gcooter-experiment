...
const MainLayout: React.FC = () => {
  ...
  const markers = vehicles.map((vehicle) => ({id: vehicle.id, latlng: {latitude: Number.parseFloat(vehicle.lat), longitude: Number.parseFloat(vehicle.lng)}, title: vehicle.title, description: vehicle.description, battery: vehicle.battery}));

  return (
    <View style={styles.container}>
      <MapView style={styles.map} showsUserLocation={true} initialRegion={initialRegion}>
        {markers.map((marker, index) => (
          <MarkerWrap key={marker.id} onPress={() => setSelectedMarker(marker.id)}>
            <Marker
